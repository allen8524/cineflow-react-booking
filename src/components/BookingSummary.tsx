  ...
  return (
    <aside className="booking-summary seat-summary-panel">
      <div className="screening-summary-title">
        <h2>예매 요약</h2>
        <span className="summary-chip">SUMMARY</span>
      </div>
      ...
      <div className="screening-summary-grid summary-grid-compact">
        ...
        <div className="summary-info-block">
          <span>좌석</span>
          <strong>{draft.selectedSeats.length > 0 ? draft.selectedSeats.join(', ') : '선택 없음'}</strong>
        </div>
      </div>
      ...
      {showAction ? (
        <Link
          to="/payment"
          className={`hero-btn primary summary-submit ${totalPeople > 0 && draft.selectedSeats.length === totalPeople ? '' : 'disabled'}`}
        >
          결제 단계로 이동
        </Link>
      ) : null}
